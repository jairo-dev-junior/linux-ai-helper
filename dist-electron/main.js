...
function graphics(callback) {
  ...
  function getNvidiaSmi() {
    ...
    if (_windows$b) {
      try {
        const basePath = util$c.WINDIR + String.raw`\System32\DriverStore\FileRepository`;
        const candidateDirs = fs$4.readdirSync(basePath).filter((dir) => {
          if (fs$4.statSync([basePath, dir].join("/")).isDirectory()) {
  ...
  function nvidiaSmi(options) {
    ...
    if (nvidiaSmiExe) {
      ...
      try {
        const sanitized = util$c.sanitizeShellString(cmd) + (_linux$a ? "  2>/dev/null" : "");
...
${BSDName}|"; smartctl -H ${BSDName} | grep overall;`;
      ...
      if (_darwin$9) {
        exec$9("system_profiler SPSerialATADataType SPNVMeDataType SPUSBDataType", { maxBuffer: 1024 * 1024 }, (error, stdout) => {
          if (!error) {
            const lines = stdout.toString().split("\n");
            const linesSATA = [];
            const linesNVMe = [];
...
ipcMain.handle("get-system-data", async () => {
  try {
    const [systemInfo, osInfo2, cpuInfo, memInfo, networkInterfaces2, timeInfo] = await Promise.all([
      si.system(),
      si.osInfo(),
      si.cpu(),
      si.mem(),
    ...
    let mainIP = "N/A";
    const interfaces = [];
    for (const netInterface of networkInterfaces2) {
    ...
    const diagnostic = {
      sistemaOperacional: {
        tipo: osInfo2.platform || "Desconhecido",
        plataforma: osInfo2.platform || "Desconhecido",
        arquitetura: require$$0$1.arch(),
        versao: osInfo2.distro ? `${osInfo2.distro} ${osInfo2.release || ""}`.trim() : osInfo2.release || "Desconhecido",
        release: osInfo2.kernel || osInfo2.release || "Desconhecido",
        hostname: osInfo2.hostname || "Desconhecido"
      },
      processador: {
      ...
      armazenamento: {
        diretorioHome: require$$0$1.homedir(),
        diretorioTmp: require$$0$1.tmpdir()
      },
      sistema: {
        tempoAtividade: formatUptime(timeInfo.uptime || 0),
        usuario: require$$0$1.userInfo().username,
        enderecoIP: mainIP
      },
